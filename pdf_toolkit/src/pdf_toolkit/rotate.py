...
def rotate_pdf_pages(
    ...
    if out_pdf.exists() and not overwrite and out_pdf.resolve() != pdf_path.resolve():
        # For a separate output file, we skip unless --overwrite is set.
        ...
        recorder.write_manifest(
            manifest_path,
            summary={"status": "skipped", "reason": "output exists"},
        )
        return
    ...
    try:
        ...
        raise UserError(f"Failed to rotate PDF {pdf_path}: {exc}") from exc

    if temp_path is not None:
        try:
            temp_path.replace(out_pdf)
            recorder.log(f"Replaced original PDF with rotated file: {out_pdf}")
        except Exception as exc:  # pragma: no cover - rare filesystem errors
            raise UserError(f"Failed to replace original PDF: {exc}") from exc
...
def rotate_images_in_folder(
    ...
    if not files:
        ...
        recorder.write_manifest(
            manifest_path, summary={"status": "no-matches", "files_found": 0}
        )
        return
